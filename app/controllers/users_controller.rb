...
@q = User.where("work_range LIKE ? OR work_range LIKE ? OR work_range LIKE ? OR work_range LIKE ? OR work_range LIKE ? OR work_range LIKE ?", "%定住%", "%永住%", "%配偶者%", "%permanent long-term spouse%", "%日本%", "%Japanese%").where(status: "sms").ransack(params[:q])
@users = @q.result(distinct: true)
           .includes(:comments)
           .order(updated_at: :desc)
           .paginate(page: params[:page], per_page: 150)
