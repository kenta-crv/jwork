class ClientMailer < ActionMailer::Base
  ...
  def received_email(client)
    ...
    mail(subject: "#{client.company} から申し込みがありました。") do |format|
      format.text
    end
  end
  ...
  def inquiry_received_email(client)
    @client = client
    mail from: client.email
    mail to: "info@j-work.jp"
    mail(subject: "#{client.company} から問い合わせがありました。") do |format|
      format.text
    end
  end

  def inquiry_send_email(client)
    @client = client
    mail to: client.email
    mail(subject: '外国人転職『J Work』にお問い合わせ頂きありがとうございます。') do |format|
      format.text
    end
  end
